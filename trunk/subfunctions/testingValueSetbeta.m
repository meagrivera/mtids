...
%             noOfInputsToMux = size( getNumericValue( Vars{1},handles ),2 ) - noOfIntInputs;
        ...
        %Adapt "Selectors" according to specified inputs
%         disp('SelectorTest')
%         noOfInputsToMux = size( getNumericValue( Vars,handles ),2 ) - noOfIntInputs
%         if ~isempty( Vars )
%             ii = 1
%             initialindex = 1
%             endindex = noOfInputsToMux;
%             for ii = 1 : size(Data,1)
%                 if regexp(Data{ii,1},'Selector')>0
%                     disp('SelectorExists')
%                     vec = initialindex:endindex
%                     Data{ii,1}
%                     set_param([handles.sysname '_tempCopy/' Data{ii,1} ], 'InputPortWidth', num2str(2*noOfInputsToMux) );
%                     set_param([handles.sysname '_tempCopy/' Data{ii,1} ], 'Indices', num2str(initialindex:endindex) );
%                     initialindex = initialindex + noOfInputsToMux
%                     endindex = endindex+noOfInputsToMux
%                     x = get_param([handles.sysname '_tempCopy/' Data{ii,1} ], 'InputPortWidth')
%                     y = get_param([handles.sysname '_tempCopy/' Data{ii,1} ], 'Indices')
%                 end
%             end
%      
%         end
        ...
        warning('on','all');
        %If/then statement to determine if there are multiple output types,
        %i.e. x and y
        if size( simout.get('xoutNew'),2 ) >= size( simout.get('youtNew'),2 )
            ...
            dimension.templateoutputs = size( simout.get('youtNew'),2 );
        end
        %-------
        if ~isempty( simout )
            if ask4choice
