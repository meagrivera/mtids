...
%Define structure data.newTemplate (PDK)

% data = getappdata(handles.figure1,'appData');
% wholetemplatelist=get(handles.selector_dynamics,'String');
% n_template=get(handles.selector_dynamics,'Value');
% data.newTemplate{1,1} = wholetemplatelist{n_template};
% data.newTemplate{1,2} = data.template_list{n_template,4};
% data.newTemplate
...
if any( ~cellfun( @isempty, templ{ 1,2 }.inputSpec.Vars ) )
    ...
    if ~isCorrect
        ...
        disp(['For node ' num2str(data.nodenumber) ': States variable(s) "' errStrg '" possess(es) incorrect dimension(s).']);
    else
...
%     consistency_display(isCorrect,error)
    if ~isCorrect
        ...
        errStrg = errStrg(3:end);
        disp(['For node ' num2str(data.nodenumber) ': Output variable(s) "' errStrg '" possess(es) incorrect dimension(s).']);
    else
