...
Any connection made inside MTIDS is bidirectional and unweighted. As Simulink connections are directed, each MTIDS bidirectional connection
 is represented with two directed connections between nodes. To imlement weighted graphs, weights 
...
model name, list of nodes' dynamics templates,list of templates that are availible in mtids, adjacense matrix, position of the nodes ant the nodes' names. 
...
The kuramoto template is an example for a non-linear dynamic of a node. The kuramoto model is used to model the behavior of coupled oscillators. One of the interesting behaviors
that may occur in this type of systems is synchronisation. The mathematical equation of $N$ coupled oscillators is written for each oscillator $i$ as
...
\caption[MTIDS Kuramoto Template]{Kuramoto template for fireflies synchronisation}
\label{templateKuramotoFig}
\end{figure}
 
In Figure \ref{templateKuramotoFig} we see  the MTIDS's kuramoto template after formula \ref{kuramotoFor}. The goal was to produce fireflies synchronisation, thus
...
Dynamics $\rightarrow$ Add mdl template. You also need to set up the right parameters for synchronisation, an example can be seen in Figure \ref{syncFig}.
In order to see the blinking in the correct time scale, you need to set up the simalation with a fixed step size in Simulink, more on this in section 
...
\caption[MTIDS fireflies synchronisation]{Simulation of 10 fireflies synchronizing. Parameters: K=2, N=10, $\omega=0.314$, $\theta_0=1$, var=100 (variation of starting point), thershhold= $3/4*2\pi$}
\label{syncFig}
\end{figure}
...
that regulates the data communication of one layer/cluster to other layers/cluster. The interface has acces to all nodes in the current cluster/layer and 
...
In oder to create a layer/cluster in MTIDS you need to export your subsystem as a layer: \textbf{File $\rightarrow$ Export as a layer}.
MTIDS will ask you to select a template for the interface node. You may select a customize inteface node created by you or select the MTIDS provided
interface node : \textbf{layerInterface.mdl}
The result is a Simulink model that differs from the one that MTIDS normally produced, as it includes the interface node.
...
\subsection{Build your own Interface Node Template }
...
As we see in Figure \ref{interfaceFig} the interface node requires a layer in and out port numbered as one, which interact with other layers/clusters. 
Moreover it requires the basic of node template (an inport connected to a mux and an outport) in order to interact with the nodes in the cluster/layer.
MTIDS also come preloaded with a simple aggregating interface template define called \textbf{layerInterface.mdl}.
 


 

\section{Working in Simuling}\label{workinginsimulink}
...
define the parameters in the Simulink model as variables and give a value to this variables in the Matlab workspace. THis also aloows you to save this parameters
by saving the workspace as a mat file with the command: \texttt{>> save name.mat}

\subsection{Solver} \label{solver}
Before runing a simulation you need to define the simulation runtime and the numerical solver that Simulink will use to simulate your model.
under Simulink: Simulation $\rightarrow$ Configure Parameters, and then going to the solver tab, you may define different types of solver, see Figure \ref{simulink1Fig}.
...
Simulation $\rightarrow$ Configure Parameters at the tab of Import/Export, see Figure \ref{simulink2Fig}. Here is useful to export the states which Simulink defines as the 
values coming out of integrator blocks, but it is also posible to define them by hand.
...
one may produce with MTIDS. One can write a matlab script that parametrize the simulation runs it and plots the results. An example script could be:
 


\section{Import from Simulink}

MTIDS also offers the posibility of importing simulink models which were produced by MTIDS or are contructed following the MTIDS format, 
see section \ref{exportToSimulink}.\\
\textbf{Notice that the dynamics of the imported will be the one that you have selcted in the MTIDS gui.}
\\
...
combines tools used for graph theory analysis (Matgraph) and the simulation of interconnected systems (Simulink) were combine to create a flexible and extendible 
framework to make the simulation of interconnedted dynamical systems easier for students and researchers.\\
