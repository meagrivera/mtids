...
\subsection{Node dynamics and mdl-templates}
Node dynamics are implemented as Simulink model files, this allows for implementation of systems which are not only limited to the traditional linear state-space models, but also to nonlinear and switching dynamics.
\subsubsection{The MTIDS-Format for templates}

\subsection{Layering and clustering}


\section{User Interface}
\subsubsection{Opening MTIDS}
Inside Matlab, change the current directory to the one which contains the \emph{mtids.m} script
Run the \emph{mtids.m} script
...
\subsection{Drag-and-Drop operations}
\subsubsection{Adding nodes}
Nodes are added using \verb|<Shift>-<Left click>|. This adds a new node with label and dynamics as selected on the ``node creation`` section. 
\subsubsection{Drag nodes}
Nodes can be dragged and repositioned on the screen by holding down the \verb|<Left click>| buttom on the mouse. 
\subsubsection{Add/remove connections}
Connections are added or removed using \verb|<Ctrl>-<Left click>| on both nodes. If a connection doesn't exist, it will be created, otherwise it will remove this connection. 
\subsection{Access node properties}
The node properties window is accessed by a \verb|<Double Left click>| on the corresponding node. 



\subsection{Add mdl-templates}
\subsection{Adding and removing nodes}
\subsection{Adding and removing connections}

\subsection{Editting node properties}



\section{Interacting with Simulink}
\subsection{Exporting to Simulink}
\subsection{Creating and modifying templates}
\subsection{Layering}
\subsection{Simulation}


\section{Interacting with Matlab}
\subsection{Linear Algebra}
\subsection{Eigenvector and eigenvalue extraction of matrices}
\subsubsection{Ranks of matrices and nullspaces}
\subsection{Control Toolbox functions}
\subsubsection{Linearizing and testing for controlability}
